...
def use_pca_latent_spaces(latent_spaces, n_components=180):
    ...
    """
    n_samples, n_features = latent_spaces.shape
...
def save_to_dataframe(file_names, latent_spaces_pca):
    ...
    converts the latent spaces to a suitable format, and saves them to a dataframe, 
    which is then saved to an HDF5 file.
    """
    latent_spaces_list = [ls.tolist() for ls in latent_spaces_pca]
    df = pd.DataFrame({'path': file_names, 'latent_space': latent_spaces_list})
    df.to_hdf('data/latent_spaces.h5', key='df_items', mode='w')

def main():
    """
    This function orchestrates the whole process of loading images, extracting latent spaces, 
    scaling and applying PCA, and saving the results.
    """
